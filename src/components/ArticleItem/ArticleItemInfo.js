...
export default function ArticleItemInfo({
  ...
  const { dateFormated, dateRelated } = formatedDate(date)
  return (
    <div className="article-item__info">
      {
        isShowAvatar && <ArticleItemAvatar slugAuthorLink={slugAuthorLink} avatar={author?.avatar} nickname={author?.nickname} />
      }
      <div className="article-item__info-right">
        <div className="article-item__author-name">
          <Link to={slugAuthorLink}><strong>{author?.nickname}</strong></Link>
        </div>
